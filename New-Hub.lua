...
shared.LoaderTitle = "Script Made By Qminh";
...
local v2 = {
    LoaderData = {
        ...
        Colors = shared.LoaderColors or {
            ...
            LoaderBackground = Color3.fromRGB(135, 206, 235),
...
local v7 = CreateObject("ImageLabel", {
    ...
    Image = "rbxassetid://108644522770850",
...
Window = Fluent:CreateWindow({
    Title = "Ân Nguyễn Hub-Blox Fruit [ Free ]",
    SubTitle = "| By Qminh",
    TabWidth = 160,
...
task.spawn(function()
    while task.wait(0.1) do
        if not getgenv().AutoFactory or not World2 then continue
        end
...
local ChestToggle = ESP:AddToggle("ChestToggle", {Title = "ESP Chest", Default = false})
ChestToggle:OnChanged(function(Value)
    ChestESP = Value
    if ChestESP then
        if not chestConnection then
            chestConnection = RunService.Heartbeat:Connect(function()
                for _, chest in pairs(CollectionService:GetTagged("_ChestTagged")) do
                    pcall(function()
                        -- nếu chest bị xóa/disable thì remove ESP
                        if not chest or not chest.Parent or chest:GetAttribute("IsDisabled") then
                        ...
                        if not chest:FindFirstChild("ChestEsp") then
                            ...
                            name.BackgroundTransparency = 1
                            name.TextStrokeTransparency = 0.5
                            name.TextColor3 = Color3.fromRGB(255, 215, 0) -- vàng
                        else
                            local distance = round((player.Character.Head.Position - chest:GetPivot().Position).Magnitude / 3)
...
local flowerConnection
local FlowerToggle = ESP:AddToggle("FlowerToggle", {Title = "ESP Flowers", Default = false})
FlowerToggle:OnChanged(function(Value)
    FlowerESP = Value
    if FlowerESP then
        if not flowerConnection then
            flowerConnection = RunService.Heartbeat:Connect(function()
                        ...
                        if v.Name == "Flower1" or v.Name == "Flower2" then
                            if not v:FindFirstChild("FlowerEsp") then
                                ...
                                v.FlowerEsp.TextLabel.Text = v.Name .. "\n" ..
                                    round((player.Character.Head.Position - v.Position).Magnitude/3) .. " Distance"
                            end
                        end
                    end)
                end
        ...
        if flowerConnection then
            flowerConnection:Disconnect()
            flowerConnection = nil
        end
        for _, v in pairs(workspace:GetChildren()) do
...
Button.Image = "rbxassetid://108644522770850"
